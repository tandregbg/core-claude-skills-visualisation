...
function renderThreadedFiles(files) {
    ...
    for (const m of mains) {
        // Find a prep whose key is a prefix of this main's key, or vice versa
        let bestPrep = null;
...
async function loadFolderTasks(documentPath) {
    ...
    try {
        ...
        for (const t of active) {
            ...
            html += `<tr>
                <td><button class="task-done-btn" onclick="docCompleteTask(event, ${t.id}, '${sourceFile}')" title="Mark as done">&#10003;</button></td>
...
window.docCompleteTask = async function(event, taskId, sourceFile) {
    ...
    try {
        ...
        if (res.ok && data.status === 'ok') {
            if (selectedFilePath) {
                loadFolderTasks(selectedFilePath);
            }
        } else {
